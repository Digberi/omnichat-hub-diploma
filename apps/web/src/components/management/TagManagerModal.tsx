            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  if (editingTag) await updateTag(editingTag.id, payload);
                  else await createTag(payload);
                  cancel();
                }}
              >
                {editingTag ? 'Зберегти' : 'Створити'}
              </Button>
            </div>
          </div>
        )}
        ...
        <div className="space-y-1 max-h-60 overflow-y-auto scrollbar-thin">
          {tags.map((tag) => (
            ...
            >
              ...
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => startEdit(tag)}>
              ...
              <Button
                ...
                onClick={async () => {
                  if (!window.confirm(`Видалити тег "${tag.name}"?`)) return;
                  await deleteTag(tag.id);
                  if (editingTag?.id === tag.id) cancel();
                }}
              >
