...
// dev/local where they live in apps/workers/.env (or repo-root .env).
// Production (DO App Platform) injects env at the runtime level so this is
// a no-op there. Nest's ConfigModule re-reads later — that's fine, dotenv
...
// can install its module-load hooks for http, pg, ioredis, etc. Gated on
// OBS_ENABLED=1 inside bootstrap; at the default OBS_ENABLED=0 it returns
// early and is a no-op.
import { bootstrap as bootstrapObs, isStarted as obsIsStarted } from "@omnichat/observability/node"

void bootstrapObs({ serviceName: "omnichat-workers" }).catch((err) => {
...
async function bootstrap() {
  ...
  if (sentryDsn) {
    ...
    // We want frames like: app:///apps/workers/dist/main.js
    const monorepoRoot = resolve(__dirname, "../../..")

    // OTel <-> Sentry bridge: tags Sentry events with otel.trace_id / otel.span_id
    // and stamps the active span with sentry.event_id. See apps/api main.ts.
  ...
  const port = Number(config.get("WORKERS_PORT") ?? 4122)
