  ...
  imports: [
    LoggerModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => {
        ...
        // independently of the local stdout level (e.g. ship `info` to Loki
        // while local stdout stays at `warn`, or vice-versa). Falls back to
        // the standard LOG_LEVEL when unset.
        ...
        if (obsEnabled) {
          ...
          // stdout/pretty pipeline intact. The OTel stream relies on the global
          // LoggerProvider set by bootstrap(); if bootstrap was skipped
          // (OBS_ENABLED=0 was the runtime view), the noop logger is used and
          // this is a graceful no-op.
          //
          // For prod we deliberately bypass pino.transport() (= worker thread)
          // for the stdout target and use a plain Writable that fans to
          // process.stdout. Mixing a ThreadStream and a regular Writable in
          // pino.multistream causes the regular Writable to never receive
          // log lines (collector observed 1401 spans but 0 log records from
          // api/workers in prod, with no errors anywhere). For dev, we still
          // want pino-pretty's worker, so we keep the transport path.
          // Pass the provider directly to avoid the fragile global-state path:
          // bootstrap() sets logs.setGlobalLoggerProvider but in our prod
          // runtime logger.emit() was a silent no-op (logs.getLogger likely
          // resolved a competing NoopLoggerProvider). Direct provider binding
          // is the recommended pattern from the OTel ecosystem.
