...
import { PromConnectDto } from "./dto/prom-connect.dto"
...
@Controller("channel-accounts/prom")
export class PromConnectController {
  constructor(private readonly prom: PromConnectService) {}

  @Post("connect")
  @Version("1")
  @ApiOkResponse({ type: ChannelAccountDto })
  async connect(@CurrentUser() user: AccessTokenPayload, @Body() body: PromConnectDto) {
    const account = await this.prom.connect({
